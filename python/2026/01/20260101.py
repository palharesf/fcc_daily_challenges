...
def resolution_streak(days):
    for i in range(len(days)):
        if days[i][0] < 10000 or days[i][1] > 120 or days[i][2] < 5:
            return "Resolution failed on day " + str(i + 1) + ": " + str(i) + " day streak."
    return "Resolution on track: " + str(len(days)) + " day streak."
...
# Comments - not too difficult. For each subarray, we check all three conditions at once. If any fails at any point, we return the failure message considering how many days were successful prior to the failure.
# Otherwise, we iterate through the whole array and return the success message considering all days that were analyzed
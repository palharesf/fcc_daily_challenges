...
def knight_moves(position):
    variations = [[1, 2], [1, -2], [-1, 2], [-1, -2], [2, 1], [2, -1], [-2, 1], [-2, -1]]
    row = int(position[1]) - 1
    col = ord(position[0]) - 65
    count = 0
    for variation in variations:
        if 0 <= row + variation[0] < 8 and 0 <= col + variation[1] < 8:
            count += 1
    return count
...
# Comments - good teamwork with AI. I thought of creating the variations array, which shows all potential moves
# AI then thought of converting letters to an index via the ord function. After that the problem was pretty much done
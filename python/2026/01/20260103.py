    ...
    for index, row in enumerate(table):
        if index == 0:
            for seat_index, seat in enumerate(row):
                if seat == "U":
                    # Left is to the RIGHT in the array (seat_index + 1)
                    if seat_index == len(row) - 1 or row[seat_index + 1] != "R":
                        count += 1
        else:
            for seat_index, seat in enumerate(row):
                if seat == "U":
                    # Left is to the LEFT in the array (seat_index - 1)
...
# Comments - cannot say it was an enjoyable exercise, but the nested loop structure was good practice. Matrix challenges tend to be complicated and this was not an exception
# Looping throughout the matrix wasn't the hard part thou; the mirroring validity checks for what a valid seat is was the hardest part to codify and test 
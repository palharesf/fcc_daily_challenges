...
def get_number_of_plants(field_size, unit, crop):
    field_size_meters = field_size * 4046.86 if unit == "acres" else field_size * 10000
    space_per_plant = 1 if crop == "corn" else 0.1 if crop == "wheat" else 0.5 if crop == "soybeans" else 0.25 if crop == "tomatoes" else 0.2
    return int(field_size_meters / space_per_plant)
...
# Simple implementation, we just need to map which unit and which crop we're working with, the rest is simple math
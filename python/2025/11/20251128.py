...
def compare(word, guess):
    # Initialize result list with '0's
    result = ['0'] * len(word)

    # Create a list to track used letters in the secret word
    used = [False] * len(word)

    for i in range(len(word)):
        # First pass: Check for exact matches ('2')
        if guess[i] == word[i]:
            result[i] = '2'
            used[i] = True
    
    for i in range(len(word)):
            if result[i] == '0':  # Only check positions not already matched
                for j in range(len(word)):
                    if not used[j] and guess[i] == word[j]:
                        result[i] = '1'
                        used[j] = True
                        break  # Stop after finding first available match

    return "".join(result)

print(compare("APPLE", "POPPA"))        # should return "10201"
print(compare("REACT", "TRACE"))        # should return "11221"
print(compare("DEBUGS", "PYTHON"))      # should return "000000"
print(compare("JAVASCRIPT", "TYPESCRIPT")) # should return "0000222222"
print(compare("ORANGE", "ROUNDS"))      # should return "110200"
print(compare("WIRELESS", "ETHERNET"))  # should return "10021000"

# Comments: a pretty difficult challenge. The first pass is relatively easy, but the second pass requires two nested loops to track which letters have already been used, which is not too difficult but can be confusing
...
def adjust_thermostat(current_f, target_c):
    ...
    if target_f > current_f:
        return f"Heat: {round(target_f - current_f, 1)} degrees Fahrenheit"
    elif target_f < current_f:
        return f"Cool: {round(current_f - target_f, 1)} degrees Fahrenheit"
    else:
        return "Hold"

adjust_thermostat(32, 0)
adjust_thermostat(70, 25)
adjust_thermostat(72, 18)
adjust_thermostat(212, 100)
adjust_thermostat(59, 22)

# Comments: very easy exercise. Basically two steps: doing the temp conversion, and then comparing the target and current temps and returning the properly f-string output for each case